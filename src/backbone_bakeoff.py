...
BAKEOFF_CSV = utils.LOGS_DIR / "backbone_bakeoff.csv"


def run_one(name: str, image_size: int, device, log) -> dict:
    result_path = utils.LOGS_DIR / f"bakeoff_{name}_result.json"
    ...
    best_meta, best_epoch, elapsed, _ = fit(
        ...
        history_csv=utils.LOGS_DIR / f"bakeoff_{name}_history.csv",
